...
from app.api.schemas import Card, GroundedCard, TaskAnalysis, GenerateInput, GeneratedCard, FieldSource
...
async def analyze(description: str):
    return await structured(TaskAnalysis, '\nIdentify missing fields and ask 3–7 DISTINCT relevant actionable clarification questions. Do not repeat questions.', {'description': description})


async def generate_with_sources(payload: GenerateInput) -> GeneratedCard:
...
Topic may be an explicit industry word from the source. Title may be a short source excerpt.''', payload.model_dump())
    ...
    for field in Card.model_fields:
        ...
        if values[field]:
            index = next(i for i, source in enumerate(sources) if item.value in source)
            evidence[field] = FieldSource(quote=item.value,
                label='Первоначальное описание' if index == 0 else f'Ответ на вопрос № {index}',
                question=None if index == 0 else payload.answers[index - 1].question)
    return GeneratedCard(card=Card(**values), sources=evidence)


async def generate(payload: GenerateInput) -> Card:
    return (await generate_with_sources(payload)).card
