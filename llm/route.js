...
async function handleAnalyze(req,res){
 ...
 if(!(req.headers['content-type']||'').toLowerCase().startsWith('application/json'))return send(415,{error:'Используйте Content-Type: application/json.'});
 ...
 try{
  ...
  const result=req.url.split('?')[0]==='/api/llm/card'?buildCard(input):await analyze(input);send(200,result);
