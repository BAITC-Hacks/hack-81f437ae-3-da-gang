...
test("live OpenAI demo from description to accepted proposal", async ({
  page,
}) => {
  ...
  const analysisResponse = page.waitForResponse(
    (response) =>
      response.url().endsWith("/api/tasks/analyze") &&
  ...
  expect(new Set(analysis.questions).size).toBe(analysis.questions.length);
  await expect(
    page.getByRole("heading", { name: "Уточним важные детали" }),
  ...
  await answers
    .first()
    .fill(
      "Пользователи — администраторы клиники. Остальные сведения пока неизвестны.",
    );
  await page.getByRole("button", { name: "Сформировать карточку" }).click();
  await expect(
    page.getByRole("heading", { name: "Карточка вашей задачи" }),
  ).toBeVisible({ timeout: 100000 });
  ...
  const before = Number(
    (await page.locator(".score").innerText()).split("/")[0],
  );
  ...
  const fields = {
    "Отрасль / тема": "Здравоохранение",
    Контекст: "Пациенты ожидают приёма в клинике.",
    "Потребность бизнеса": "Сократить очереди.",
    Пользователи: "Администраторы и пациенты.",
    "Данные и материалы": "Обезличенный журнал приёмов за месяц.",
    Ограничения: "Без персональных данных, прототип за 2 недели.",
    "Ожидаемый результат": "Прототип расписания.",
    "Критерии успеха": "Сократить среднее ожидание на 20%.",
    Контакт: "clinic-demo@example.com",
    "Формат взаимодействия": "Еженедельный созвон.",
  };
  ...
  await expect(page.getByRole("heading", { name: title })).toBeVisible();
  const scores = (
    await page.locator(".task-card .badge").allTextContents()
  ).map((text) => Number.parseInt(text));
  expect(scores).toEqual([...scores].sort((a, b) => b - a));
  await page
    .getByLabel("Отрасль", { exact: true })
    .selectOption("Здравоохранение");
  await expect
    .poll(async () => {
      const topics = await page.locator(".task-card .topic").allTextContents();
      return (
        topics.length > 0 &&
        topics.every((topic) => topic === "Здравоохранение")
      );
    })
    .toBe(true);
  await page.getByLabel("Готовность", { exact: true }).selectOption("priority");
  await expect
    .poll(async () => {
      const ratings = await page.locator(".task-card .badge").allTextContents();
      return (
        ratings.length > 0 &&
        ratings.every((text) => Number.parseInt(text) >= 90)
      );
    })
    .toBe(true);
  await expect(page.getByRole("heading", { name: title })).toBeVisible();
  await page.screenshot({
    path: "../.tools/catalog-desktop.png",
    fullPage: true,
  });
  await page.setViewportSize({ width: 390, height: 844 });
  await page.screenshot({
    path: "../.tools/catalog-mobile.png",
    fullPage: true,
  });
  expect(
    await page.evaluate(
  ...
  await page
    ...
    .fill("Анализ журнала, прототип, проверка с клиникой");
