...
import { RoleProvider, RoleSwitch, BusinessOnly, useRole } from "./roles";
import "./styles.css";
import { QualityPanel, ProposalCoach, PilotEditor } from "./workbench";
    ...
    <BrowserRouter>
      ...
      <main>
        <Routes>
          ...
          <Route
            path="/business/proposals/:id/pilot"
            element={
              <BusinessOnly>
                <PilotEditor />
              </BusinessOnly>
...
function Create() {
  ...
  if (card) return <Editor initial={card} sources={sources} />;
  return (
          ...
          onSubmit={(e) => {
            ...
            void run(async () => {
              if (!analysis) {
                ...
                const generated = await api<GeneratedCard>(
                  "/tasks/generate-with-sources",
                  "POST",
                  {
                    description,
...
function Editor({
  ...
  const [initialScore, setInitialScore] = useState<number | null>(
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      api<Rating>("/tasks/preview", "POST", card, controller.signal)
        .then((result) => {
          setRating(result);
          setInitialScore((previous) => previous ?? result.score);
        })
        .catch((e) => {
  ...
  return (
    <>
      ...
      {rating && initialScore !== null && (
        <div className="journey">
          <span>При открытии: {initialScore}/100</span>
          <span>Сейчас: {rating.score}/100</span>
          <span className="active">
            {rating.score > initialScore
              ? `+${rating.score - initialScore} баллов готовности`
              : "Уточняйте задачу шаг за шагом"}
          </span>
        </div>
      )}
      <QualityPanel
        ...
        apply={(next) => {
          ...
          setDirty(true);
          setNotice(
            "Уточнения перенесены. Проверьте карточку и подтвердите сведения.",
          );
        }}
      />
              ...
              <div key={key} className="editor-field">
                ...
                {sources[key] && (
                  <details className="field-source">
                    <summary>
                      {(card[key] || "").trim() === sources[key]!.quote.trim()
                        ? "Источник AI-поля"
                        : "Изменено вручную · исходная цитата"}
                      : {sources[key]!.label}
                    </summary>
                    {sources[key]!.question && <p>{sources[key]!.question}</p>}
                    <blockquote>{sources[key]!.quote}</blockquote>
                  </details>
                )}
              </div>
            ))}
  ...
  return (
    <>
      ...
      <div className="actions no-print">
        <button onClick={() => window.print()}>Печать карточки / PDF</button>
      </div>
      {role === "business" && (
        <div className="actions">
      ...
      <div className="editor-layout">
        ...
        <section className="panel proposal-panel no-print">
          <p className="eyebrow">ДЛЯ СТУДЕНЧЕСКИХ КОМАНД</p>
          <h2>Предложите решение</h2>
          {role === "business" ? (
            <>
              <p>Посмотрите, как команда предложит решение вашей задачи.</p>
              <button onClick={() => setRole("team")}>
                Перейти в режим команды
              </button>
            </>
            ...
            >
              ...
              <ProposalCoach
                ...
                proposal={proposal}
                disabled={busy}
                apply={(solution_idea, plan) =>
                  setProposal({ ...proposal, solution_idea, plan })
                }
              />
              <ErrorBox message={error} />
...
function Proposals() {
  ...
  useEffect(() => {
    ...
    // StrictMode/navigation must not let an old GET overwrite a newer decision.
    return () => controller.abort();
  }, [id]);
  async function decision(item: Proposal, status: "accepted" | "rejected") {
    await run(async () => {
      ...
      setItems((current) =>
        current.map((p) => (p.id === changed.id ? changed : p)),
      );
    });
  }
  return (
    <>
      ...
      <div className="comparison-toolbar">
        <span>
          Всего: {items.length} · Принято:{" "}
          {items.filter((p) => p.status === "accepted").length} · Ожидают:{" "}
          {items.filter((p) => p.status === "pending").length}
        </span>
        <div className="role-switch" aria-label="Вид предложений">
          <button
      ...
      {loading ? (
        <p>Загрузка…</p>
      ) : loadError ? null : !error && !items.length ? (
        <div className="empty">
        ...
        >
          <table className="comparison-table">
            <caption>
              Сравните подходы и выберите команды самостоятельно
            </caption>
            <thead>
              <tr>
                {[
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>{item.solution_idea}</td>
                  <td>{item.plan}</td>
                  <td>{item.estimated_time}</td>
                  <td>
                    {item.prototype_url ? (
                      <a
                        href={item.prototype_url}
                        target="_blank"
                        rel="noreferrer"
                      >
                        Открыть ↗
                  ...
                  <td>
                    ...
                    {item.status === "accepted" && (
                      <Link
                        ...
                        to={`/business/proposals/${item.id}/pilot`}
                      >
                        Подготовить пилот →
                      </Link>
                    )}
